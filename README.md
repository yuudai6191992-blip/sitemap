...
- **🗺️ 風景マップ表示（メイン）**：山・土砂／河川／橋・道路／都市／沿岸 などの「ゾーン」で構成した風景の上に、技術をアイコンで配置。ホバーで名称、クリックで詳細を表示
- **ゾーンで絞り込み**：風景上部のゾーンチップ（複数選択可）で、その分野の技術だけを強調表示
...
- **レスポンシブ対応**：PC / タブレット / スマートフォン（狭い画面では風景を横スクロール、または一覧表示）
...
│   └── map.js            … MapManager（風景マップのゾーン・ピン描画）
├── data/
